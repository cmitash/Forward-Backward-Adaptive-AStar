...
int fwd_astar(Point start, Point end, vector<vector <char> > &graph, vector<vector <int> > &astar_gcount, int astar_count, vector<Point> &res)
{
	init_astar();
	res.clear();
	insert_hp(start);
...
int main(int argc, char *argv[])
{
	...
	while(current_position != end)
	{
		...
		if(updated || no_path)
		{
			...
			//forward astar
			ret = fwd_astar(current_position, end, current_graph, astar_gcount, num_astar, res);
			...
			/*
			vector<vector <char> > outfile(MAX_SIZE, vector<char>(MAX_SIZE, '0'));
			for(int i=0;i<res.size();i++)
			{
				outfile[res[i].x][res[i].y] = '*';
				output_file << res[i].x<<" "<<res[i].y<<endl;
			}
